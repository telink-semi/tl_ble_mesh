 ...
 */
#if (1) /* used in IRQ, avoid flash code, TODO: optimize it latter */
...
_attribute_data_retention_sec_	rf_mode_e   g_rfmode;
...
void rf_set_ant_mode(void)
{
	...
	write_reg8(0x140803,0x44);//bit<0:1>private mode control. bit<2:3> tx mode.
	write_reg8(0x140804,0xfb);//bit<4>mode:1->1m;bit<0:3>: head.
...
{
	//TODO: can optimize, now 316 Byte(rf_set_chn inlined)
...
void rf_set_access_code_len(unsigned char byte_len)
{
	unsigned char temp;
	temp = byte_len & 0x07;
	write_reg8(0x140805,read_reg8(0x140805)|temp);
}
 ...
 * @param[in]	millisec	-	rx_timeout after timeout_us us,The maximum of this param is 0xfff.
 * @return  none.
 */
void rf_set_rx_timeout(unsigned short timeout_us)
{
	...
	timeout_us &= 0x0fff;
	tmp = read_reg16(0x140a0a) & 0xf000;
	tmp |= timeout_us;
	write_reg16(0x140a0a, tmp);
}
...
{
	retry_times &= 0x0f; //accommodate with  chips
