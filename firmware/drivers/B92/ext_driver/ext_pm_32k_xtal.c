...
_attribute_ram_code_ int cpu_sleep_wakeup_32k_xtal(SleepMode_TypeDef sleep_mode,  SleepWakeupSrc_TypeDef wakeup_src, unsigned int  wakeup_tick)
{
	...
	if(timer_wakeup_enable){
		...
		else
		{
			...
			if(span >= 0x0ff00000){  //BIT(28) = 0x08000000   16M:8.389S
				g_pm_long_suspend = 1;
			}
			else{
				g_pm_long_suspend = 0;
	...
	if(wakeup_src & PM_TIM_RECOVER_START){
		pm_timRecover.tick_sysClk = g_pm_tick_cur;
		pm_timRecover.tick_32k = g_pm_tick_32k_cur;
	}
...
//		tick_adjust_us = ((wakeup_tick - g_pm_tick_cur)/800000) * 16 * SYSTEM_TIMER_TICK_1US;//50ms -- 16us
		earlyWakeup_us = g_pm_early_wakeup_time_us.deep_ret_early_wakeup_time_us - 30;
	}
...
//		tick_adjust_us = ((wakeup_tick - g_pm_tick_cur)/800000) * 16 * SYSTEM_TIMER_TICK_1US;//50ms -- 16us
	...
	if(g_pm_long_suspend){
		tick_reset = g_pm_tick_32k_cur + (unsigned int)(tick_wakeup_reset -tick_adjust_us - g_pm_tick_cur)/ CRYSTAL32768_TICK_PER_32CYCLE * 32;
	}
	else{
		tick_reset = g_pm_tick_32k_cur + ((unsigned int)(tick_wakeup_reset -tick_adjust_us - g_pm_tick_cur) * 32 + (CRYSTAL32768_TICK_PER_32CYCLE>>1)) / CRYSTAL32768_TICK_PER_32CYCLE;
	}
	...
	if( (wakeup_src & PM_TIM_RECOVER_END) && pm_timRecover.recover_flag){
		...
		int rest_32k_cycle = (int )(tick_reset_timRecover - g_pm_tick_32k_cur);
	...
	if(pm_timRecover.recover_flag == 0x03){
		...
		if(g_pm_long_suspend){
			g_pm_tick_cur = pm_timRecover.tick_sysClk+ (unsigned int)(now_tick_32k - pm_timRecover.tick_32k) / 32 * CRYSTAL32768_TICK_PER_32CYCLE;
		}
		else{
			g_pm_tick_cur = pm_timRecover.tick_sysClk + (unsigned int)(now_tick_32k - pm_timRecover.tick_32k) * CRYSTAL32768_TICK_PER_32CYCLE / 32;		// current clock
		}
	}
	else
#endif
