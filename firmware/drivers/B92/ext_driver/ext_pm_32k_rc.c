...
//	tick_reset = g_pm_tick_32k_cur + ((unsigned int)(wakeup_tick - g_pm_tick_cur) * 16 + tick_32k_halfCalib - tick_32k_halfCalib ) / g_pm_tick_32k_calib  - 1 - 3*2 - 2;
	...
	if( (wakeup_src & PM_TIM_RECOVER_END) && pm_timRecover.recover_flag){
		...
		if(g_pm_long_suspend){
			tick_reset_timRecover = pm_timRecover.tick_32k + (unsigned int)(tick_wakeup_reset - pm_timRecover.tick_sysClk)/ g_pm_tick_32k_calib * 16;
		}
		else{
			tick_reset_timRecover = pm_timRecover.tick_32k + ((unsigned int)(tick_wakeup_reset - pm_timRecover.tick_sysClk) * 16 + tick_32k_halfCalib) / g_pm_tick_32k_calib;
		}
	...
	if(pm_timRecover.recover_flag == 0x03){
		if(g_pm_long_suspend){
			g_pm_tick_cur = pm_timRecover.tick_sysClk+ (unsigned int)(now_tick_32k - pm_timRecover.tick_32k) / 16 * g_pm_tick_32k_calib;
		}
		else{
			g_pm_tick_cur = pm_timRecover.tick_sysClk + (unsigned int)(now_tick_32k - pm_timRecover.tick_32k) * g_pm_tick_32k_calib / 16;		// current clock
		}
	}
	else
#endif
...
#if (PM_TIM_RECOVER_MODE)
	if(pm_timRecover.recover_flag == 0x03){
		deepRet_tick = pm_timRecover.tick_sysClk + (unsigned int)(now_tick_32k - pm_timRecover.tick_32k) * g_pm_tick_32k_calib / 16;		// current clock
	}
	else
#endif
	{
