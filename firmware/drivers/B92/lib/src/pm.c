...
extern _attribute_ram_code_sec_noinline_ void flash_send_cmd(unsigned char cmd);
...
void pm_set_wakeup_time_param(pm_r_delay_cycle_s param)
{
	...
	g_pm_early_wakeup_time_us.suspend_early_wakeup_time_us = suspend_ret_rx_delay_us + 109 + 200 + 225;
...
_attribute_ram_code_sec_noinline_ void  pm_sleep_start(void)
{
	...
	flash_send_cmd(0xb9);	//flash deep
...
#endif
		//0xA3FFFF20 mspi_set_l: mutiboot address offset option, 0:0k;  1:128k;  2:256k;  4:512k
		//0xA3FFFF21 mspi_set_h: program space size = mspi_set_h*4k
