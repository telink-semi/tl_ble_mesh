	...
	else if(kb_event.cnt == 1)
	{
		if(key0 >= CR_VOL_UP )  //volume up/down
		{
			...
			if(key0 == CR_VOL_UP){  	//volume up
				...
				my_dump_str_data(APP_DBUG_LOG, "volume up", 0, 0);
			}
			else if(key0 == CR_VOL_DN){ //volume down
				consumer_key = MKEY_VOL_DN;
				my_dump_str_data(APP_DBUG_LOG, "volume down", 0, 0);
			}
			...
			if(key0 == BTN_PAIR)   //Manual pair triggered by Key Press
			{
				...
				my_dump_str_data(APP_DBUG_LOG, "Manual pair triggered", 0, 0);
			}
			else if(key0 == BTN_UNPAIR) //Manual un_pair triggered by Key Press
			{
				my_dump_str_data(APP_DBUG_LOG, "Manual un_pair triggered", 0, 0);
