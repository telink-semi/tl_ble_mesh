		...
		u8 ext_evtType = pExtAdvInfo->event_type & EXTADV_RPT_EVTTYPE_MASK;
			...
			if(clock_time_exceed(tick_adv_rpt, 2000000)){  //2000000
				...
				if(ext_evtType & EXTADV_RPT_EVT_MASK_LEGACY){
					tlkapi_send_string_data(APP_DUMP_EN, "Adv report legacy", pExtAdvInfo->address, 6);
				}
				else{
					tlkapi_send_string_data(APP_DUMP_EN, "Adv report extend", pExtAdvInfo->address, 6);
				}
			}
		#endif
		...
		if(ext_evtType == EXTADV_RPT_EVTTYPE_LEGACY_ADV_IND || ext_evtType == EXTADV_RPT_EVTTYPE_EXT_CONNECTABLE_UNDIRECTED)
		{
			...
			if(central_auto_connect || user_manual_pairing){
				...
				if(status == BLE_SUCCESS){ //create connection success
						...
						tlkapi_send_string_data(APP_DUMP_EN, "Ext Create connection success", pExtAdvInfo->address, 6);
				}
				else{
					tlkapi_send_string_data(APP_DUMP_EN, "Ext Create connection fail", &status, 1);
				}
			}
		}
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	tlkapi_send_string_data(APP_DUMP_EN, "cis central init", NULL, 0);
}
