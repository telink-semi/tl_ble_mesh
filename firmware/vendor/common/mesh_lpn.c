	...
	if(is_lpn_support_and_en && (BLS_LINK_STATE_CONN != blc_ll_getCurrentState())){
		...
		if(CMD_CTL_REQUEST == op){
            ...
            if(get_lpn_poll_interval_ms() > LPN_LONG_SLEEP_THRES_MS){
                ...
                bls_pm_setSuspendMask (SUSPEND_DISABLE); // disable ble pm, will enter long sleep in lpn_check_and_enter_sleep().
            }
            else{
...
int mesh_lpn_adv_interval_update(u8 adv_tick_refresh)
{
	...
	if(adv_tick_refresh){
		...
		blt_advExpectTime = clock_time() + (blc_ll_getAdvInterval() * 1000 * 100 / 160 * sys_tick_per_us); // blc_ll_getAdvInterval() unit is 0.625ms(100/160)
	}
#endif
