...
_attribute_data_retention_ u32 flash_sector_mac_address    = CFG_SECTOR_ADR_MAC_CODE; // BLE_SRC_TELINK_MESH_EN
_attribute_data_retention_ u32 flash_sector_calibration    = CFG_SECTOR_ADR_CALIBRATION_CODE; // BLE_SRC_TELINK_MESH_EN
...
void blc_readFlashSize_autoConfigCustomFlashSector(void)
{
    ...
    if(CFG_SECTOR_ADR_MAC_CODE != flash_sector_mac_address){
        u8 cfg_mac_read[8];
        u8 cap_mac_read[8];
        u8 ff_six_byte[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
        flash_read_page(CFG_SECTOR_ADR_MAC_CODE, 8, cfg_mac_read);
        flash_read_page(flash_sector_mac_address, 8, cap_mac_read);

        // only use flash capacity mac when the user config mac is empty and the flash capacity mac is not. otherwise use the user config address.
        if(!((0 == memcmp(cfg_mac_read, ff_six_byte, 6)) && memcmp(cap_mac_read, ff_six_byte, 6))){
            flash_sector_mac_address = CFG_SECTOR_ADR_MAC_CODE;
