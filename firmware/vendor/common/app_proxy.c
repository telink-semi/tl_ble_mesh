...
int	pb_gatt_Write (u16 connHandle, void *p)
{
	...
	u8 service_uuid[] = WRAPPING_BRACES(SIG_MESH_PROXY_SERVICE);
	...
	dispatch_pb_gatt(connHandle, proxy_para_buf ,proxy_para_len);
	#endif 
	return 0;
}
...
int proxy_gatt_Write(u16 connHandle, void *p)
{
	...
	if(p_gatt->msgType == MSG_PROXY_CONFIG ){
		...
		if(0 == mesh_rc_data_cfg_gatt((u8 *)p_bear)){
		    proxy_config_dispatch(connHandle,(u8 *)&p_bear->nw,proxy_para_len);
		}
...
void proxy_cfg_list_init_upon_connection(u8 *p)
{
	hci_le_connectionCompleteEvt_t *pConnEvt = (hci_le_connectionCompleteEvt_t *)p;
	if(pConnEvt->status == BLE_SUCCESS){
		if(pConnEvt->role == ACL_ROLE_PERIPHERAL) // central role, process SMP and SDP if necessary
		{
			int idx = get_slave_idx_by_conn_handle(pConnEvt->connHandle);
			memset(&proxy_mag[idx], 0x00, sizeof(proxy_config_mag_str));
...
int is_addr_in_proxy_list(int idx, u16 adr)
{
	return (-1 != find_data_in_list(idx, adr));
}

int is_valid_with_filter_list(int idx, u16 adr)
{
	int valid = (FILTER_WHITE_LIST == proxy_mag[idx].filter_type) ? is_addr_in_proxy_list(idx, adr):!is_addr_in_proxy_list(idx, adr);
	return valid;
}

int is_valid_adv_with_proxy_filter(u16 dst_adr)
{
	int valid = 1;
	if(dst_adr == PROXY_CONFIG_FILTER_DST_ADR){
	}else{
		foreach(idx, ACL_PERIPHR_MAX_NUM){
			valid = is_valid_with_filter_list(idx, dst_adr);
			if(valid){
				break;
			}
		}
...
int add_data_to_list(int idx, u16 src)
{
	if(get_list_cnt(idx)>=MAX_LIST_LEN){
		return -1;
	}
	
    if(is_addr_in_proxy_list(idx, src)){
        return 0;
    }
	
	foreach(i, MAX_LIST_LEN){
		if(0 == proxy_mag[idx].addr_list[i]){
			proxy_mag[idx].addr_list[i] = src;
			return 0;
		}
	}
...
u8 proxy_proc_filter_mesh_cmd(int idx, u16 src)
{
	if(idx < ACL_PERIPHR_MAX_NUM){
	    if(proxy_mag[idx].filter_type == FILTER_WHITE_LIST){
	        return add_data_to_list(idx, src);
	    }else if (proxy_mag[idx].filter_type == FILTER_BLACK_LIST){
	        return delete_data_from_list(idx, src);
	    }else{
...
u8 proxy_config_dispatch(u16 connHandle, u8 *p, u8 len )
{
	...
	int idx = get_slave_idx_by_conn_handle(connHandle);
	switch(p_str->opcode & 0x3f){
		case PROXY_FILTER_SET_TYPE:
			...
			if(FILTER_WHITE_LIST ==  p_str->para[0]){
				add_data_to_list(idx, app_adr); // add app_adr for find connHandle to report filter status later
			}
