...
//	u8 key_buf[8] = {0,0,0,0,0,0,0,0};
	...
	else if(kb_event.cnt == 1)
	{
		if(key0 >= CR_VOL_UP )  //volume up/down
		{
			...
			if (key0 == BTN_PAIR) {
				...
				tlkapi_send_string_data(APP_DUMP_EN, "blc_ll_setAdvEnable", &adv_status, 1);
				...
				for (int i = ACL_CENTRAL_MAX_NUM; i < (ACL_CENTRAL_MAX_NUM + ACL_PERIPHR_MAX_NUM); i++) { //peripheral index is from "ACL_CENTRAL_MAX_NUM" to "ACL_CENTRAL_MAX_NUM + ACL_PERIPHR_MAX_NUM - 1"
					if (conn_dev_list[i].conn_state) {
						...
						tlkapi_send_string_data(APP_DUMP_EN, "bls_l2cap_requestConnParamUpdate  latency", &latency, 2);
						if (latency) {
							latency = 0;
						}
						else {
	...
	else   //kb_event.cnt == 0,  key release
	{
		...
		if (key_type == CONSUMER_KEY) {
			...
			for (int i = ACL_CENTRAL_MAX_NUM; i < (ACL_CENTRAL_MAX_NUM + ACL_PERIPHR_MAX_NUM); i++) { //peripheral index is from "ACL_CENTRAL_MAX_NUM" to "ACL_CENTRAL_MAX_NUM + ACL_PERIPHR_MAX_NUM - 1"
				if (conn_dev_list[i].conn_state) {
					tlkapi_send_string_data(APP_DUMP_EN, "blc_gatt_pushHandleValueNotify", &consumer_key, 2);
					blc_gatt_pushHandleValueNotify(conn_dev_list[i].conn_handle, HID_CONSUME_REPORT_INPUT_DP_H, (u8*) &consumer_key, 2);
				}
			}
		}
