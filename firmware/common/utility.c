...
u8 * my_fifo_get (my_fifo_t *f) // BLE_SRC_TELINK_MESH_EN
{
    u8 *p = 0;
...
u8 my_fifo_data_cnt_get (my_fifo_t *f) // BLE_SRC_TELINK_MESH_EN
{
    return (f->wptr - f->rptr);
}

u8 my_fifo_free_cnt_get(my_fifo_t *f) // BLE_SRC_TELINK_MESH_EN
{
    return (f->num - my_fifo_data_cnt_get(f));
}

void my_fifo_reset(my_fifo_t *f) // BLE_SRC_TELINK_MESH_EN
{
    f->rptr = f->wptr;
}
